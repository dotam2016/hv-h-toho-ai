...
function Loader() {
  ...
  return <AnimatePresence>{visible && <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0, transition: { duration: .8, ease: "easeInOut" } }} className="fixed inset-0 z-[90] grid place-items-center bg-ink">
    <div className="relative flex flex-col items-center gap-8">
      <motion.div initial={{ scale: .92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: .7, ease: "easeOut" }}><Image src="/media/logo.svg" alt="TOHO" width={180} height={54} className="h-auto w-40" /></motion.div>
...
function Nav() {
  ...
  return <>
    <header className="fixed left-0 right-0 top-0 z-50 flex items-center justify-between px-5 py-4 md:px-8">
      <a href="#top" className={`flex h-11 items-center rounded-full border px-4 backdrop-blur-xl transition-colors duration-300 ${navShell}`} aria-label="TOHO 電動バイク トップ">
        <Image src="/media/logo.svg" alt="FOLMO" width={92} height={28} className={`h-5 w-auto transition duration-300 ${logoTone}`} />
