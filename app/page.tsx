...
const images = [
  { src: "/media/hero-commute-helmet.png", label: "都市を軽く抜ける" },
...
function Hero() {
  ...
  return <section id="top" ref={ref} data-nav-theme="dark" className="relative min-h-[112vh] overflow-hidden">
    <motion.div style={{ scale }} className="absolute inset-0"><Image src="/media/hero-commute-helmet.png" alt="都市を走るTOHO電動バイク" fill priority sizes="100vw" className="object-cover" /></motion.div>
    ...
    <motion.div style={{ y }} className="relative z-10 flex min-h-screen flex-col justify-end px-5 pb-20 md:px-10 lg:px-16">
      ...
      <h1 className="max-w-6xl text-[15vw] font-black leading-[.86] tracking-normal md:text-[8.2vw]">街を、<br />スマートに走る</h1>
...
function Specs() { return <section id="specs" data-nav-theme="dark" className="bg-ink py-28 md:py-40"><div className="mx-auto max-w-7xl px-5 md:px-10"><div className="grid gap-10 md:grid-cols-[1.1fr_.9fr]"><div data-reveal className="relative min-h-[560px] overflow-hidden rounded-lg border border-white/10"><Image src="/media/spec-product-latest.png" alt="TOHO電動バイクの製品写真" fill sizes="(min-width: 768px) 52vw, 100vw" className="object-cover" /></div><div><p data-reveal className="text-sm font-bold uppercase tracking-[.35em] text-signal">Specifications</p><h2 data-reveal className="mt-6 text-5xl font-black leading-none md:text-7xl">毎日の移動を支える、<br />確かなスペック</h2><dl className="mt-10 divide-y divide-white/12">{specs.map(([k,v])=><div data-reveal key={k} className="grid grid-cols-[.9fr_1.2fr] gap-4 py-4 text-sm"><dt className="text-white/48">{k}</dt><dd className="font-semibold text-white/88">{v}</dd></div>)}</dl><p className="mt-6 text-xs leading-relaxed text-white/42">※仕様・デザインは予告なく変更される場合があります。画像内資料に基づくプロトタイプ表記です。</p></div></div></div></section>; }

function Campaign() { return <section id="crowdfunding" data-nav-theme="light" className="relative overflow-hidden bg-mist py-28 text-ink md:py-36"><div className="mx-auto max-w-7xl px-5 md:px-10"><div className="grid gap-10 md:grid-cols-[1fr_1fr]"><div><p data-reveal className="text-sm font-bold uppercase tracking-[.35em] text-black/42">Crowdfunding</p><h2 data-reveal className="mt-6 text-6xl font-black leading-[.9] md:text-8xl">走り出す日は、<br />もうすぐ</h2></div><div data-reveal className="glass !bg-black/[.06] rounded-lg p-7 text-ink"><p className="text-xl leading-relaxed">2026年7月中旬〜下旬、クラウドファンディング開始予定。2026年10月ごろ、公式サイト販売開始予定。</p><form className="mt-8 grid gap-3 sm:grid-cols-[1fr_auto]"><input aria-label="メールアドレス" className="rounded-full border border-black/15 bg-white px-5 py-4 text-ink outline-none" placeholder="メールアドレス" /><button className="rounded-full bg-ink px-7 py-4 font-bold text-white transition hover:bg-black">先行案内を登録</button></form></div></div></div><div className="mt-20 flex w-[200%] gap-8 text-7xl font-black text-black/10 marquee"><span>FREEDOM / URBAN / COMPACT / ELECTRIC / TOHO /</span><span>FREEDOM / URBAN / COMPACT / ELECTRIC / TOHO /</span></div></section>; }
