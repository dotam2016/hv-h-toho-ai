...
function Specs() { return <section id="specs" data-nav-theme="dark" className="bg-ink py-28 md:py-40"><div className="mx-auto max-w-7xl max-w-[96rem] px-5 md:px-10"><div className="grid gap-10 md:grid-cols-[1.1fr_.9fr]"><div data-reveal className="relative min-h-[560px] overflow-hidden rounded-lg border border-white/10"><Image src="/media/spec-product-latest.png" alt="TOHO電動バイクの製品写真" fill sizes="(min-width: 768px) 52vw, 100vw" className="object-cover" /></div><div><p data-reveal className="text-sm font-bold uppercase tracking-[.35em] text-signal">Specifications</p><h2 data-reveal className="mt-6 text-5xl font-black leading-none md:text-7xl">走る、止まる、しまう。必要な性能を一枚に。</h2><dl className="mt-10 divide-y divide-white/12">{specs.map(([k,v])=><div data-reveal key={k} className="grid grid-cols-[.9fr_1.2fr] gap-4 py-4 text-sm"><dt className="text-white/48">{k}</dt><dd className="font-semibold text-white/88">{v}</dd></div>)}</dl><p className="mt-6 text-xs leading-relaxed text-white/42">※仕様・デザインは予告なく変更される場合があります。画像内資料に基づくプロトタイプ表記です。</p></div></div></div></section>; }
