...
const images = [
  { src: "/media/city-ride.png", label: "都市を軽く抜ける" },
...
function Nav() {
  ...
  return <>
    <header className="fixed left-0 right-0 top-0 z-50 flex items-center justify-between px-5 py-4 md:px-8">
      <a href="#top" className="glass flex h-11 items-center rounded-full px-4" aria-label="TOHO 電動バイク トップ"><Image src="/media/toho-logo.png" alt="TOHO" width={92} height={28} className="h-5 w-auto" /></a>
      <nav className="glass hidden items-center gap-1 rounded-full p-1 text-sm text-white/72 md:flex">
        {nav.map((n) => <a key={n} href={"#" + n.toLowerCase()} className="rounded-full px-4 py-2 transition hover:bg-white/12 hover:text-white">{n}</a>)}
      </nav>
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => setOpen(true)} className="glass grid h-11 w-11 place-items-center rounded-full md:hidden" aria-label="メニューを開く"><Menu size={20} /></button>
      </div>
    </header>
...
function Hero() {
  ...
  return <section id="top" ref={ref} className="relative min-h-[112vh] overflow-hidden">
    <motion.div style={{ scale }} className="absolute inset-0"><Image src="/media/city-ride.png" alt="都市を走るTOHO電動バイク" fill priority className="object-cover" /></motion.div>
